...
export default function HomeScreen({ navigation }) {
  ...
  const liveEvents = events.filter(e => e.type === 'live' && e.live_status !== 'ended');
  const rest = events.filter(e => e.type !== 'live' || e.live_status === 'ended');
...
const styles = StyleSheet.create({
  ...
  author: { color: '#555', fontSize: 12, marginTop: 6 },
  statusBtns: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 6,
